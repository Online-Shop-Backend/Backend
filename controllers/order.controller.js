...
//  * Priority: req.user?._id (JWT), then req.body.user_id, then req.query.user_id
//  */
// function resolveUserId(req) {
//   return req?.user?._id || req?.body?.user_id || req?.query?.user_id;
// }

// /**
//  * POST /api/orders/checkout
//  * Body: { address_id: string, user_id?: string }
//  * - Creates Order from the user's Cart
//  * - Creates OrderItems
//  * - Decrements Product stock
//  * - Clears CartItems
//  */
...
//   if (!userId) return res.status(400).json({ error: "user_id is required for now (temporary until auth is wired)." });
...
//       // 2) Load all products referenced by cart items
//       const productIds = cartItems.map((ci) => ci.product_id);
//       const products = await Product.find({ _id: { $in: productIds } }).session(session);
//       const productMap = new Map(products.map((p) => [p._id.toString(), p]));

//       // 3) Validate stock first
//       for (const item of cartItems) {
//         const prod = productMap.get(item.product_id.toString());
//         if (!prod) throw new Error("Product not found for a cart item.");
//         if (prod.stock < item.quantity) {
//           throw new Error(`Insufficient stock for "${prod.name}". Requested: ${item.quantity}, In stock: ${prod.stock}`);
//         }
//       }

//       // 4) Create the order (total = 0 for now; we’ll compute and update)
...
//       // 5) Create order items, decrement stock, compute total
//       let total = 0;
//       const orderItemsPayload = [];

//       for (const item of cartItems) {
//         const prod = productMap.get(item.product_id.toString());
//         const price = item.price_at_add ?? prod.price; // snapshot at add if present; else current
...
//       createdItems = await OrderItem.insertMany(orderItemsPayload, { session });

//       // 6) Update order total
//       createdOrder.total = Math.round((total + Number.EPSILON) * 100) / 100;
...
// /**
//  * GET /api/orders/my  (optionally ?user_id=xxx for now)
//  * Returns list of orders for the current user with their items.
//  */
...
//   try {
//     // Aggregate orders with joined order items & products
...
// /**
//  * GET /api/orders/:orderId
//  * Returns one order with its items (+ product snapshots).
//  */
// export const getOrderById = async (req, res) => {
//   const { orderId } = req.params;
